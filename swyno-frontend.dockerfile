...
# na pasta atual (/app), pois normalmente ele criaria uma pasta para o projeto (/app/swyno-frontend)
RUN git clone --depth 1 --branch ${TAG} https://github.com/Ozaru/swyno-frontend.git .
...
# este comando abre a porta 4200 (porta padrão do projeto) para que possa ser acessada de fora
EXPOSE 4200
